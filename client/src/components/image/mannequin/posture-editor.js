...
function exportPosture()
{
    ...
    const parsed = parser.parseFromString(svgfile, 'image/svg+xml')
    const paths = parsed.querySelector('svg').innerHTML

	window.top.postMessage(svgfile, '*')    //TODO:fix this

	/*
	var svgData = svgfile;
	var preface = '<?xml version="1.0" standalone="no"?>\r\n';
	var svgStr = preface + svgData
